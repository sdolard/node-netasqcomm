...
Session = function (config) {
	...
	*/
	...
	/**
	* Request list, array of binded function (connect, disconnect, exec...)
	*/
	this._requestList = [];
...
Session.prototype._runRequest = function() {
	...
	this._currentRequest = this._requestList.shift();
	this._currentRequest();
};

Session.prototype._afterRequestCallback = function() {
	this._currentRequest = undefined;
...
Session.prototype._request = function (config, callback) {
	...
	r.on('response', function(response) {
			...
			function onData(chunk) {
				...
				// Other type mime
				data === undefined ? data = chunk : data += chunk;
...
Session.prototype.connect = function(cb) {
	this._requestList.push(this._authenticate.bind(this, cb));
	this._runRequest();
};
...
Session.prototype.disconnect = function(cb) {
	this._requestList.push(this._exec.bind(this, 'quit', cb));
	this._runRequest();
};
...
Session.prototype.exec = function(cmd, cb) {
	this._requestList.push(this._exec.bind(this, cmd, cb));
	this._runRequest();
};

/**
* @private
* @see Session.prototype.exec
*/
Session.prototype._exec = function(cmd, cb, connectAfterRequest) {
	if (connectAfterRequest === undefined) {
		connectAfterRequest = true;
	}
	...
	if (cb) {
		if (this.listeners('exec').length !== 0) {
			return this.emit('error', this._createError({
					code: 'EEXECLISTENERERROR',
					message: 'Exec should not have any listeners on new command'
			}));
		}

		this.once('exec', cb);
	}
	if (connectAfterRequest) {
		this.once('exec', this._afterRequestCallback.bind(this));
	}
...
Session.prototype.upload = function(fileName, cb) {
	this._requestList.push(this._upload.bind(this, fileName, cb));
	this._runRequest();
};

/**
* @private
* @see Session.prototype.upload
*/
Session.prototype._upload = function(fileName, cb){
	//console.log('_upload: ' + fileName);
	var
	me = this;
	fileName = fileName || '';
	...
	this.once('upload', this._afterRequestCallback.bind(this));
...
Session.prototype._authenticate = function (cb) {
	...
	this.once('connect', this._afterRequestCallback.bind(this));
...
* @params {string} fileName, default to 'defaultFileName.txt'. Path should exists before.
...
Session.prototype.download = function(wStream, fileName, cb) {
	this._requestList.push(this._download.bind(this, wStream, fileName, cb));
	this._runRequest();
};

/**
* @private
* @see Session.prototype.download
*/
Session.prototype._download = function(wStream, fileName, cb, connectAfterRequest){
	var	options;
	...
	if (connectAfterRequest) {
		this.once('download', this._afterRequestCallback.bind(this));
	}
	if (!this._authenticated) {
	...
	this._request(options, function (err, sessionDataResponse){
			this.emit('download', err, sessionDataResponse);
	}.bind(this));

};


Session.prototype._createDirIfNotExists = function(dir, cb) {

	fs.stat(dir, function(err, stats){
		if (err) { // no exists
			this.log('_createDirIfNotExists: creating %s', dir);
...
Session.prototype.downloadBackup = function(fileName, what, cb) {
	this._requestList.push(this._downloadBackup.bind(this, fileName, what, cb));
	this._runRequest();
};

/**
* @private
* @see Session.prototype.downloadBackup
*/
Session.prototype._downloadBackup = function(fileName, what, cb){
	what = what || 'all';

	//console.log('_downloadBackup: ' + fileName);

	if (cb) {
		this.once('backup', cb);
	}
	this.once('backup', this._afterRequestCallback.bind(this));
	...
	this._exec(cmd, this._onDownloadBackupResponse.bind(this, fileName), false);
};
...
Session.prototype._downloadToFile = function(fileName, size, crc, event) {
	this._createDirIfNotExists(path.dirname(fileName), function(err) {
		if (err) {
			return this.emit(event, err);
		}
		...
		fileWs.on('error', function (exception) {
				this.log('fileWs error', exception);
				this.emit(event, exception);
		}.bind(this));
		this.log('Download pending...');
		this._download(fileWs, fileName, function(err, response){
				this.log('%s downloaded.', fileName);
				this.log('Checking file size...');

				fs.stat(fileName, function(err, stats) {
						if (!err) { // no erro, "something" exists
							if (stats.size !== parseInt(size, 10)) {
								this.emit(event, this._createError({
										message: util.format('File size is not valid: %do instead of %do!', stats.size, size),
										code: 'EINVALIDSIZE'
								}), size, crc);
							} else  {
								this.log('File size is valid.');
								this.emit(event, undefined, size, crc);
							}
						} else {
							this.emit(event, this._createError({

...
Session.prototype._request = function (options, callback) { 
	var 
	me = this,
	r;
	...
	// ssl or not?
	r = me.ssl ? https.request(options): http.request(options);
	...
	r.on('response', function(response) {
			...
			if (response.headers['content-type'].indexOf('text/xml') !== -1) {
				...
				parser.onerror = function (e) {
					// an error happened.
					me.log('parser.onerror (Sesison._request) e:', e.message);
					me.emit('error', e);
				};
	...
	default:
		me.log("NetasqRequest unmanaged request method: %s',", options.method);
	}
...
* @param [{function} callback], optionnal, use a callback or 'connect' event
* @see 'connect' event
*/
...
Session.prototype.exec = function(cmd, cb) {
	...
	this._request(options, function (sessionDataResponse){
			me.emit('commandResponse', sessionDataResponse);
	});
};
...
Session.prototype.download = function(wStream, fileName, cb){	
	...
	this._request(options, function (sessionDataResponse){
			me.emit('downloaded', sessionDataResponse);
	});
};
			...
			if (!err) { // no erro, "something" exists
				if (stats.isFile()) {
					...
					*/
					...
					this._request(options, function (sessionDataResponse){
							me.emit('uploaded', sessionDataResponse);
					});
				}
			} else {
...
Session.prototype._createError = function(config) {
	...
	Error.captureStackTrace(error, Session.prototype._cexception); // we do not trace this function
...
Session.prototype._login = function() {
	...
	this._request(options, function (sessionDataResponse){
			...
			case sdr.NWS_ERROR_CODE.TOO_MANY_USER_AUTHENTICATED: 
				me.log('Login failed.');
				me.emit('connect', this._createError({
						code: 'ETOOMANYUSER', 
...
Session.prototype.downloadBackup = function(fileName, what, cb){
	...
	this.exec(cmd, function(response){
			var
			exception,
			fileSize,
			fileCRC,
				...
				response.dataFollow())
			{
				...
				exception = new Error(util.format('Backup failed. See response.'));
				exception.code = 'EDLFAILED';
...
Session.prototype._downloadToFile = function(fileName, size, crc, event) {
	...
	this.download(fileWs, fileName, function(response){
			...
			fs.stat(fileName, function(err, stats) {
					...
					if (!err) { // no erro, "something" exists
						if (stats.size !== parseInt(size, 10)) {
							exception = new Error(util.format('File size is not valid: %do instead of %do!', stats.size, size));
							exception.code = 'EINVALIDSIZE';
							me.log(exception.message);
						...
						exception = new Error(util.format('File %s not found!', fileName));
						exception.code = 'EFILENOTFOUND';
						me.log(exception.message);
						me.emit(event, exception, size, crc);
					}
			});
